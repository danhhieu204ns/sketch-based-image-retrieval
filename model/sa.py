...
class Self_Attention(nn.Module):
    def __init__(self, d_model=768, cls_number=100, pretrained=True):
        ...
        if pretrained:
            # Load pre-trained ViT-B/16 from Hugging Face (timm)
            pretrained_model = timm.create_model('vit_base_patch16_224', pretrained=True)
            cur = self.model.state_dict()
            pretrained_state = pretrained_model.state_dict()
            
            # Map and load compatible weights
            new = {k: v for k, v in pretrained_state.items() if k in cur.keys() and 'mlp_head' not in k}
            cur.update(new)
